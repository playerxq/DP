// hdu-1400-骨牌放置.cpp : 定义控制台应用程序的入口点。
//
...
using namespace std;
const int maxn=15;
const int ST = 4096;//0表示有骨牌 1表示没有
long long dp[maxn][maxn][ST];//dp[i][j][k]表示推到(i,j)时且(i,j)未判断时 上一行(i-1,j)到本行(i,j-1)组成的(i,1):(i,j-1)+(i-1,j):(i-1,w)共w个数组成状态k 的数目
int h,w;
void solve()
{
	for(int i = 1;i<=h+1;i++)
